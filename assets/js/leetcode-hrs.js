...
function startlcCounter() {
    const targetNumber = 60;
    const totalDuration = 2000; // Total duration in milliseconds
    const element = document.getElementById('leetcodeCounter');
    let currentNumber = 0;
...
document.addEventListener('scroll', function() {
    ...
    if (isInViewport(lcSection) && !hasStartedlcCounter) {
        hasStartedlcCounter = true;
        startlcCounter();
    }
});